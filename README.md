...
✅ **Done — the full mandate lifecycle, closed, including a real money-moving payout.** 25 real Sepolia transactions and API calls against the live sandbox (19 via `scripts/tx.mjs`'s prepare→sign→send→poll flow, a genuine agent-signed x402 payment via `scripts/x402-pay.mjs`, two via `scripts/llm-agent.mjs`'s live-LLM stress test, one autonomous revoke via `src/rules/server.ts`, and two proving RAMS's off-chain-signature lifecycle mode via `scripts/extend-via-signature.mjs`). Full log: [`docs/transactions.md`](docs/transactions.md). Live status view: **[the dashboard](https://claude.ai/code/artifact/7207bc10-1405-4ee4-ae9d-4ac41fae3e53)**.
...
**Point 6, added 2026-09-16 — a signature is not a transaction.** Every RAMS write up to this point used direct signing: the signer pays their own gas and authorizes on-chain in the same step. RAMS also supports the principal signing an EIP-712 payload **off-chain** — zero gas, no transaction from the principal at all — after which *any wallet* can broadcast it. Never tested until now. Granted a third, short-lived mandate, then extended its validity purely by signature: the Issuer signed off-chain, and the **Compliance Agent** — a wallet with zero payout power anywhere else in this system — broadcast the already-authorized change, gaining no new authority by doing so. Confirmed: `validUntil` moved by exactly the requested amount, verified against a follow-up read. Hit and fixed a real bug along the way (the typed-data envelope's numeric fields came back from the REST API as a mix of JSON strings and numbers; signing them unconverted produced a signature that didn't recover to the principal — fixed by coercing each field to its correct EIP-712 type). This is the actual precondition for cold-storage custody: the principal's key never needs to touch gas or a hot wallet, only ever produce a signature. Full detail in `docs/transactions.md`.
