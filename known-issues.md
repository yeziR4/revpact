...
- **Reported to Brickken team:** yes, 2026-09-12, with the exact repro above. **Confirmed by Brickken (Davide) on 2026-09-14**: "we're currently experiencing a server-side issue with the Base Sepolia facilitator for x402 payments. Ethereum Sepolia is working correctly" — matches our own finding exactly (the BKN faucet leg, on Ethereum Sepolia, worked; this leg, on Base Sepolia, doesn't). A fix was said to be in progress; retested the same day and the identical 500 still reproduces, so it hadn't landed yet as of this writing.
- **Net honest status:** one of the two self-funding legs (BKN faucet) is genuinely closed with a real x402-paid transaction. The other (ERC-8004 identity) is blocked by a Brickken-side server bug we found, reported, and had independently confirmed by their team — not by missing funding. Worth a quick retest (`scripts/x402-pay.mjs`) if there's time before the deadline; the fix is entirely on Brickken's side now.
